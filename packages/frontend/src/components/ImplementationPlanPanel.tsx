          ...
          {plans.slice(0, 3).map((plan) => (
            ...
            >
              ...
              {plan.ai_evaluation?.overallFeedback && (
                <p className="text-xs text-gray-500 mt-1 line-clamp-1">{plan.ai_evaluation.overallFeedback}</p>
              )}
            </div>
          ))}
              ...
              >
                ...
                {plan.ai_evaluation && (
                  <div className={`mt-3 p-3 rounded-lg text-sm ${
                    plan.status === 'ai_approved' ? 'bg-green-50 border border-green-200' :
                    plan.status === 'ai_rejected' ? 'bg-red-50 border border-red-200' :
                    plan.status === 'ai_needs_revision' ? 'bg-yellow-50 border border-yellow-200' :
                    ...
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`font-bold ${
                        ...
                        {plan.ai_evaluation_score}/100
                      </span>
                    </div>
                    {plan.ai_evaluation.overallFeedback && (
                      <p className="text-gray-600 line-clamp-2">{plan.ai_evaluation.overallFeedback}</p>
                    )}
                    <p className="text-xs text-gray-500 mt-1">Click to view full feedback</p>
                  </div>
                )}
